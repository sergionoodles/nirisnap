...
A native Wayland screenshot and annotation overlay for Niri.

Nirisnap is a Niri-focused fork of [Omasnap](https://github.com/omacom/omasnap),
pinned at `d339588f3aba554f314d6233da2dddc98f83de55` (v1.20.1). It keeps Omasnap's
fast overlay and vector-layer editor and replaces its Hyprland/Omarchy integration
with Niri JSON IPC + `wlr-screencopy`. Original MIT license/copyright and bundled
font licenses are preserved (`LICENSE`, `assets/*-OFL.txt`).
...
- Pin a finished capture as a bottom-right always-on-top layer surface, launched
  from the same `nirisnap` executable and visible on every workspace.
- Crash-resistant working documents under `/run/user/<UID>/nirisnap/` (falling back to
  a private `/tmp/nirisnap-<UID>/`): the original source image plus a sidecar JSON
...
- `tesseract` (OCR only; `NIRISNAP_OCR_LANGS`, default `eng`)
- `notify-send` when available (best-effort; notification failure never fails a save).

## Install on Niri (Arch/CachyOS)

Clone the repository and build with prefix `~/.local` (see `docs/niri-setup.md`
for binds; Nirisnap never edits your Niri config):

```bash
git clone <nirisnap-remote>
cd nirisnap
make install PREFIX="$HOME/.local"
```

It does not modify Niri configuration.

### Niri binds

Merge with your existing binds, then `niri validate` (see `docs/niri-setup.md`):
...
Layer scope is `nirisnap` (pins: `nirisnap-pin`); the overlay uses exclusive keyboard and does not rearrange tiled windows.

Set `NIRISNAP_PREFIX` (manual-prefix builds) before running `make install` to use a prefix other than
`~/.local`.
...
sudo pacman -S --needed \
  base-devel cmake ninja pkgconf qt6-base qt6-wayland layer-shell-qt \
...
- `~/.local/bin/nirisnap`
- `~/.local/share/applications/nirisnap.desktop`
- `~/.local/share/licenses/nirisnap/Neucha-OFL.txt`
- `~/.local/share/licenses/nirisnap/JetBrainsMono-OFL.txt`
- `~/.local/share/licenses/nirisnap/Inter-OFL.txt`
...
directory. Starting nirisnap while an overlay is open sends the running instance `SIGTERM`,
...
Point nirisnap at any readable image and it opens straight into the annotation editor
with the whole image selected, skipping the screen-capture step:

```bash
nirisnap ~/Pictures/Screenshots/screenshot-2026-08-11_10-00-00.png
# or
nirisnap --file /path/to/capture.png
```
...
`nirisnap` on the finished screenshot, so it can be reopened and re-annotated.
...
`~/.local/state/nirisnap/recent/` (`NIRISNAP_RECENT_DIR` overrides). The select
...
`~/.config/nirisnap/nirisnap.conf` (INI format); every key is optional:
...
| `{app}` | Slug of the app under the selection, e.g. `firefox`, `alacritty`, `nautilus` (from the window app-id; empty on Niri window-less captures). Empty for fullscreen captures, file edits, and when nothing is known — the separator before or after it is dropped too, so the default pattern gives `screenshot-2026-08-23_14-05-09.png`. |
...
Environment overrides (`NIRISNAP_SCREENSHOT_DIR` takes precedence over the config):

```bash
NIRISNAP_SCREENSHOT_DIR="$HOME/Pictures/Captures" nirisnap
NIRISNAP_OCR_LANGS="eng+deu" nirisnap
# Thai plus English:
NIRISNAP_OCR_LANGS="tha+eng" nirisnap
```

Install the corresponding Tesseract language data before adding a language to
`NIRISNAP_OCR_LANGS`. When unset, nirisnap uses `eng`
...
the runtime snapshot directory, and launches the same `nirisnap` executable in
...
```bash
NIRISNAP_PROFILE_STARTUP=1 ./build/nirisnap 2>startup.log
```
