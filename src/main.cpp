...
int main(int argc, char **argv) {
  ...
  QCoreApplication::setApplicationName(QStringLiteral("nirisnap"));
  QCoreApplication::setApplicationVersion(QString::fromLatin1(NIRISNAP_VERSION));
  QCoreApplication::setOrganizationName(QStringLiteral("Niri"));
  qputenv("QT_WAYLAND_SHELL_INTEGRATION", "layer-shell");
  // Some sessions export QT_QPA_PLATFORMTHEME=gtk3 session-wide. Honouring it
  ...
  QGuiApplication::setDesktopFileName(QStringLiteral("nirisnap"));
  ...
  parser.setApplicationDescription(QStringLiteral(
      "Native Wayland screenshot and annotation overlay for Niri.\n"
      "\n"
      "Only one capture overlay runs at a time. Starting nirisnap again while "
  ...
  QLockFile instanceLock(
      QDir(runtime).filePath(QStringLiteral("nirisnap.instance")));
  ...
  if (lockResult.signalledPid != 0)
    qInfo().noquote() << QStringLiteral("Asked the running nirisnap (pid %1) to "
                                        "quit")
  ...
  // Grab the output before the layer exists. wlr-screencopy waits for
  ...
  if (!editingImage && !targetScreen) {
    // Never silently capture one output and display its overlay on another.
    error = QStringLiteral("Captured %1 but no matching QScreen is available")
  ...
  if (!editingImage) {
    qInfo().noquote() << QStringLiteral("Captured %1 (%2x%3 logical, %4x%5 "
                                         "pixels, scale %6)")
                             .arg(capture.monitor.name)
                             .arg(capture.monitor.geometry.width())
                             .arg(capture.monitor.geometry.height())
                             .arg(capture.source.width())
                             .arg(capture.source.height())
                             .arg(capture.monitor.scale);
  }
  ...
  layerWindow->setScope(QStringLiteral("nirisnap"));
