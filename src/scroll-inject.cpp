...
// The overlay waits for the page to settle itself;
...
/// Niri exposes no queryable natural-scroll policy over IPC, so the kernel
/// (uinput) backend cannot safely pre-compensate. Return unknown so the
/// caller uses the wlr virtual pointer by default; uinput stays available
/// only when a policy becomes queryable in the future.
std::optional<bool> hyprlandNaturalScroll() { return std::nullopt; }
...
public:
  bool open(QString &error) {
    ...
    std::strncpy(setup.name, "Nirisnap Auto Scroll", sizeof(setup.name) - 1);
...
struct WlrPointer {
  ...
  void scroll(Axis axis, int notches) {
    ...
    // (order axis_discrete before axis_source; some compositors tie the source to the most recent axis), one frame.
