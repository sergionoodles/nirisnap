...
void ScrollCapturePanel::setKeyboardGrab(bool grab) {
  // On Hyprland the compositor pinned pointer focus (Niri behavior needs live validation) to a layer that holds an exclusive keyboard
...
void ScrollCapturePanel::startCapture(Mode mode, stitch::Axis axis) {
  ...
  worker->debugDir = qEnvironmentVariable("NIRISNAP_SCROLL_DEBUG_DIR");
  ...
  repaint();
  // Drop the exclusive keyboard grab for the whole capture. On Hyprland the compositor pinned
...
void ScrollCapturePanel::continueCapture() {
  ...
  // hold the keyboard, and On Hyprland the compositor pinned pointer focus (Niri behavior needs live validation) to a layer that holds
