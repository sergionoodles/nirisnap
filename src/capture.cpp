...
QString secureRuntimeDirectory() {
  ...
  if (runtime.isEmpty()) {
    runtime = QDir(QDir::tempPath())
                  .filePath(QStringLiteral("nirisnap-%1").arg(::getuid()));
  } else {
    runtime = QDir(runtime).filePath(QStringLiteral("nirisnap"));
  }
...
QString screenshotTargetPath(QString &error, const QString &appSlug) {
  // Precedence: NIRISNAP_SCREENSHOT_DIR, then [output] directory in the
  ...
  QString root = qEnvironmentVariable("NIRISNAP_SCREENSHOT_DIR");
  if (root.isEmpty())
...
bool parseMonitor(const QByteArray &json, MonitorInfo &monitor,
                  QString &error) {
  // Legacy entry point kept for tests: interpret the payload as Niri
  // focused-output JSON. For `niri msg -j outputs` maps see niri-ipc.
  NiriOutput output;
  if (!parseNiriFocusedOutput(json, output, error))
    return false;
...
QVector<WindowTarget> parseWindows(const QByteArray &json,
  ...
  // Niri IPC has no reliable tiled-window screen coordinates; never
  // synthesize rectangles. See parseNiriWindowsPlaceholder.
  return parseNiriWindowsPlaceholder(json, monitor);
}
...
bool probeFocusedMonitor(MonitorInfo &monitor, QString &error) {
  StartupTimingScope timing("niri focused-output + parse");
  NiriOutput output;
  if (!queryNiriFocusedOutput(output, error))
    return false;
  return niriOutputToMonitor(output, monitor, error);
}
...
bool captureMonitorPixels(const MonitorInfo &monitor, CaptureData &capture,
  ...
  // Window discovery is intentionally a no-op on Niri: tiled positions are
  // unavailable, so the editor offers region/full-monitor only. Keep the
  // flag for API shape; do not spawn discovery work.
  static_cast<void>(includeWindows);

  const QString testCapture = qEnvironmentVariable("NIRISNAP_TEST_CAPTURE");
...
QString recognizeText(const QImage &image, QString &error) {
  ...
  QString languages = qEnvironmentVariable("NIRISNAP_OCR_LANGS",
                                                     QStringLiteral("eng"));
...
void sendCaptureNotification(const QString &message, const QString &imagePath) {
  // Best-effort generic notification. Failure must never fail a save;
  // click-to-reopen is deferred until an action-capable implementation is
  // tested. Uses notify-send when available, silent no-op otherwise.
  QStringList arguments{QStringLiteral("--app-name"),
                        QStringLiteral("nirisnap"), message};
  if (!imagePath.isEmpty())
    arguments << QStringLiteral("--icon") << imagePath;
  arguments << QStringLiteral("--expire-time") << QStringLiteral("4500");
