 ...
 *
 *  Env: NIRISNAP_STITCH_REPLAY_DIR (frames dir), NIRISNAP_STITCH_REPLAY_OUT
 *  (output PNG), NIRISNAP_STITCH_REPLAY_AXIS (vertical|horizontal, default
...
int main(int argc, char **argv) {
  ...
  const QString dir = qEnvironmentVariable("NIRISNAP_STITCH_REPLAY_DIR");
  const QString out = qEnvironmentVariable("NIRISNAP_STITCH_REPLAY_OUT");
  const Axis axis = qEnvironmentVariable("NIRISNAP_STITCH_REPLAY_AXIS") ==
                            QStringLiteral("horizontal")
                        ? Axis::Horizontal
                        : Axis::Vertical;
  if (dir.isEmpty()) {
    std::fprintf(stderr, "NIRISNAP_STITCH_REPLAY_DIR unset\n");
    return 2;
  }
