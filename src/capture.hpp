...
struct WindowTarget {
  ...
  QString stableId;
  QString title;
  /** Window class/app-id (e.g. `firefox`, `org.gnome.Nautilus`). */
  QString appClass;
};
...
/** Bundled annotation font at Nirisnap's logical text size. */
[[nodiscard]] QFont annotationTextFont(qreal size,
                                       TextFont textFont = TextFont::Neucha);
/**
 * Discovers the focused monitor (name, geometry, scale). Fast: only one
 * `niri msg -j focused-output` call. Safe to call on the main thread to
 * position the overlay after the pixel capture has already produced a frozen
 * frame.
 */
...
/** Returns Nirisnap's private runtime directory, or empty on failure. */
...
/** Quotes a string for a shell argument (kept for tooling; notifications
 *  now use notify-send argument lists directly). */
