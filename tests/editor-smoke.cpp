...
bool runQuickOutputChecks(QString &error) {
  ...
  const QByteArray previousDir = qgetenv("NIRISNAP_SCREENSHOT_DIR");
  qputenv("NIRISNAP_SCREENSHOT_DIR", directory.path().toUtf8());
  outputError.clear();
  ...
  if (previousDir.isEmpty())
    qunsetenv("NIRISNAP_SCREENSHOT_DIR");
  else
    qputenv("NIRISNAP_SCREENSHOT_DIR", previousDir);
...
bool runScreenshotFilenameChecks(QString &error) {
  ...
  }
  {
    const QString configPath =
        QDir(directory.path()).filePath(QStringLiteral("nirisnap.conf"));
...
bool runCrashSnapshotChecks(const CaptureData &capture, QString &error) {
  ...
  const auto restoreDir = qScopeGuard(
      [&previousDir] { qputenv("NIRISNAP_SCREENSHOT_DIR", previousDir); });
...
bool runAsyncCaptureRegionSmoke(QApplication &application, QString &error) {
  ...
  const QString fakeNiri = QDir(commands.path()).filePath(QStringLiteral("niri"));
  const QByteArray niriScript =
      QByteArrayLiteral("#!/usr/bin/env bash\n"
                        "if [[ \"$1 $2\" == \"msg -j\" ]]; then\n"
                        "  shift 2\n"
                        "  if [[ \"$1\" == \"focused-output\" ]]; then\n"
                        "    printf '%s\\n' "
                        "'{\"name\":\"TEST\",\"modes\":[{\"width\":320,\"height\":240}],"
                        "\"current_mode\":0,\"logical\":{\"x\":0,\"y\":0,\"width\":320,"
                        "\"height\":240,\"scale\":1.0,\"transform\":\"Normal\"}}'\n"
                        "  elif [[ \"$1\" == \"outputs\" ]]; then\n"
                        "    printf '%s\\n' "
                        "'{\"TEST\":{\"name\":\"TEST\",\"modes\":[{\"width\":320,"
                        "\"height\":240}],\"current_mode\":0,\"logical\":{\"x\":0,"
                        "\"y\":0,\"width\":320,\"height\":240,\"scale\":1.0,"
                        "\"transform\":\"Normal\"}}}'\n"
                        "  elif [[ \"$1\" == \"workspaces\" ]]; then\n"
                        "    printf '%s\\n' "
                        "'[{\"id\":7,\"idx\":1,\"name\":null,\"output\":\"TEST\","
  ...
  qputenv("NIRISNAP_TEST_CAPTURE", sourcePath.toUtf8());
...
bool runRecentsShelfSmoke(QApplication &application, QString &error) {
  ...
  const QByteArray previousShelf = qgetenv("NIRISNAP_RECENT_DIR");
  const QByteArray previousDir = qgetenv("NIRISNAP_SCREENSHOT_DIR");
  qputenv("NIRISNAP_RECENT_DIR", shelf.path().toUtf8());
  qputenv("NIRISNAP_SCREENSHOT_DIR", screenshots.path().toUtf8());
  const auto restoreEnv = qScopeGuard([&] {
    qputenv("NIRISNAP_RECENT_DIR", previousShelf);
    if (previousDir.isEmpty())
      qunsetenv("NIRISNAP_SCREENSHOT_DIR");
    else
      qputenv("NIRISNAP_SCREENSHOT_DIR", previousDir);
...
bool runShellQuoteCheck(QString &error) {
  if (shellQuote(QStringLiteral("nirisnap")) != QStringLiteral("'nirisnap'")) {
    error = QStringLiteral("shellQuote did not wrap a simple token");
    return false;
  }
  if (shellQuote(QStringLiteral("nirisnap /tmp/a.png")) !=
      QStringLiteral("'nirisnap /tmp/a.png'")) {
...
bool runCenteredCreationSmoke(QApplication &application, QString &error) {
  ...
  // Spotlight (Nirisnap's own drag-rectangle shape) centers the same way.
  QTest::keyClick(&editor, Qt::Key_S);
...
int main(int argc, char **argv) {
  ...
  if (!liveOutputName.isEmpty()) {
    ...
    const QString liveRoot =
        argc > 1 ? QString::fromLocal8Bit(argv[1])
                 : QDir(QDir::tempPath())
                       .filePath(QStringLiteral("nirisnap-native-smoke"));
  ...
  qputenv("NIRISNAP_SCREENSHOT_DIR", savedRoot.toUtf8());
  ...
  if (!recognizeText(ocrImage, ocrError)
    ...
    return 5;
  const QByteArray savedNirisnapLangs = qgetenv("NIRISNAP_OCR_LANGS");
  qputenv("NIRISNAP_OCR_LANGS", "");
  const QString fallbackOcr = recognizeText(ocrImage, ocrError);
  if (savedNirisnapLangs.isEmpty())
    qunsetenv("NIRISNAP_OCR_LANGS");
  else
    qputenv("NIRISNAP_OCR_LANGS", savedNirisnapLangs);
  ...
  if (!QFile::exists(savedPath))
    return 70;
  if (qEnvironmentVariableIsSet("NIRISNAP_SMOKE_COPY")) {
  ...
  QString niriError;
  if (!runNiriIpcChecks(niriError)) {
    qWarning().noquote() << niriError;
    return 66;
  }
